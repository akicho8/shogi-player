...
多数の変更をまとめたメジャーリリースです。詰み判定・移動元印・思考印まわりの機能追加、スタイルエディタの大幅な刷新、モデル層のリファクタリングなどが含みます。

### 新機能

- オプションにより盤上の特定のエリアだけを表示できるようにしました。(例えば左上の5x5など)
- 星の位置をカスタマイズできるようにしました。
- 移動元印機能を実装しました。指し手の移動元を盤上に表示できます。
- 思考印（ThinkMark）にバリアント機能を追加しました。
- 詰判定と打歩詰め判定を追加しました。
- 反則ブロック時にも局面・手数・指し手の情報をイベントに含めるようにしました。
- `illegal_info` に千日手のレコードを追加しました。
- `sp_request_*` を有効にしたとき、`ev_play_mode_move` に追加情報を含めるようにしました。
...
- 持駒が空のときは `MembershipStand` コンポーネントを表示しないようにしました。
- 棋譜を正しくパースできないときに alert を表示するようにしました。
- `.ShogiPlayer` で `container-type: inline-size` を指定するようにしました。

### 修正

- プレイヤー名がアルファベットのとき縦書きにすると文字の頭が右を向いてしまう不具合を修正しました。
...
- カーソル形状を整理しました（hover: grab、持ち上げ中: grabbing）。
- 座標表示がクリックイベントを奪ってしまう問題を修正しました。
- 反転すると星の位置がずれる問題を修正しました。
- 予期しないイベントが発生していたため、持駒操作を主ボタンに限定しました。
- `Mark.create()` にインスタンスが渡されたときに、そのインスタンスを返さない不具合を修正しました。
...
- 色変換ライブラリを chroma-js から **colorjs.io に移行**しました。あわせて `GeneralMark` の色システムを hsl から oklch に移行しています。

...
モデル層のリファクタリングなど内部的な刷新が中心ですが、あまりに変更点が多く影響範囲が広いため 2.0.0 としています。
あわせて、詰み判定・移動元印・思考印まわりの機能追加や、スタイルエディタの大幅な刷新も行っています。
通常の使い方であればそのままアップデートして問題ないはずですが、何かが動かなくなっていた場合は一度 1 系に戻すか一報ください。

### 新機能

- 詰み判定を追加しました。 (デフォルトでは機能OFF)
- 詰み判定を追加したことで打歩詰め等の反則判定をできるようにしました。
...
- 思考印（ThinkMark）にバリアント機能を追加しました。(が、一般的な機能ではないのでドキュメントにも詳しく書いてない)

...
describe("SfenParser", () => {
  describe("ClassMethods", () => {
    it(".default_create", () => {
      ...
      expect(instance.to_sfen).toEqual(SfenParser.SFEN_DEFAULT)
    })
  })
  ...
  it("1手目は先手", () => {
    const instance = SfenParser.parse(SfenParser.SFEN_DEFAULT)
    expect(instance.location_by_offset(0).key).toEqual("black")
  })

  it("2手目は△", () => {
    const instance = SfenParser.parse("position sfen lnsgkgsnl/1r5b1/ppppppppp/9/9/2P6/PP1PPPPPP/1B5R1/LNSGKGSNL w - 2")
    expect(instance.location_by_offset(0).key).toEqual("white")
  })

  it("72手目は△", () => {
    const instance = SfenParser.parse("position sfen lr4knl/3g2gs1/4ppP2/p4bNpp/2pSsN3/PPPP1P2P/2N1P1G2/2G6/L1K4RL w BPs3p 72 moves 2b3c")
    expect(instance.location_by_offset(0).key).toEqual("white")
  })

  it("駒落ちの初手は△", () => {
    const instance = SfenParser.parse("position sfen lnsgkgsnl/1r5b1/ppppppppp/9/9/9/PPPPPPPPP/1B5R1/LNSGKGSNL w - 1")
    expect(instance.location_by_offset(0).key).toEqual("white")
  })

  it("駒落ちの3手目は△", () => {
    const instance = SfenParser.parse("position sfen lnsgkgsnl/1r5b1/ppppppppp/9/9/9/PPPPPPPPP/1B5R1/LNSGKGSNL w - 3")
    expect(instance.location_by_offset(0).key).toEqual("white")
  })

  // it("指定の手数以降を求める例", () => {
  //   const instance = SfenParser.parse("position sfen 7nl/7k1/9/7pp/6N2/9/9/9/9 b GS2r2b3g3s2n3l16p 1 moves S*2c 2b1c 2c1b+ 1c1b G*2c")

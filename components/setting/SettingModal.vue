...
.modal-card.has-text-left(style="width:auto")
  header.modal-card-head
    p.modal-card-title.is-size-6 設定
    button.delete(aria-label="close" @click.stop.prevent="close_handle")

  section.modal-card-body
    b-tabs(type="is-boxed" size="is-small" v-model="tab_index")
      b-tab-item(label="基本")
        ...
        b-field(grouped)
          b-field(custom-class="is-small" label="視点")
            ...
            b-radio-button(size="is-small" v-model="TheSp.new_viewpoint" native-value="white") ☖

          b-field(custom-class="is-small" label="デバッグモード")
            b-radio-button(size="is-small" v-model="TheSp.new_debug" :native-value="false") OFF
            b-radio-button(size="is-small" v-model="TheSp.new_debug" :native-value="true") ON

          b-field(custom-class="is-small" label="イベントログ")
            b-radio-button(size="is-small" v-model="TheSp.new_event_log" :native-value="false") OFF
            b-radio-button(size="is-small" v-model="TheSp.new_event_log" :native-value="true")  ON
      b-tab-item(label="その他")
        template(v-if="false")
        ...
        //-     b-radio-button(size="is-small" v-model="TheSp.new_sp_piece_variant" :native-value="e.key") {{e.name}}

        template(v-if="TheSp.xcontainer")
          b-field(custom-class="is-small" label="再生モードの現局面(Readonly)")
            b-input(:value="TheSp.xcontainer.to_short_sfen" type="input" size="is-small" readonly)

        b-field(custom-class="is-small" label="編集モードの現局面(Readonly) ※BUG:駒を反転したときに反映されない場合がある")
          b-input(:value="TheSp.edit_mode_short_sfen2" type="input" size="is-small" readonly)

        b-field(custom-class="is-small" label="再生モードの棋譜(Readonly)")
          b-input(:value="TheSp.kifu_source" type="textarea" size="is-small" readonly)

        b-field(custom-class="is-small" label="操作モードの棋譜(Readonly)")
          b-input.is-small(:value="TheSp.play_mode_full_moves_sfen" type="textarea" size="is-small" readonly)

  footer.modal-card-foot
    button.button.is-primary.is-small(@click.stop.prevent="close_handle") 閉じる
</template>

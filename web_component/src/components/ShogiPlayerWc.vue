...
export default {
  ...
  props: {
    ...
    sp_coordinate_variant_h:           { type: String,  }, // 座標の表記
    ...
    sp_legal_move_only:                { type: Boolean, }, // play で駒の移動を制限する
    ...
    sp_board_view_x: { type: Number, }, // 視野 左上(X)
    sp_board_view_y: { type: Number, }, // 視野 左上(y)
    sp_board_view_w: { type: Number, }, // 視野 幅(W)
    sp_board_view_h: { type: Number, }, // 視野 高さ(W)
  ...
  computed: {
    ...
    override_props() {
      return {
        ...
        sp_think_mark_collection:     this.sp_think_mark_collection_native,
        sp_origin_mark_collection:    this.sp_origin_mark_collection_native,
    ...
    // sp_think_mark_collection の Array 化
    sp_think_mark_collection_native() {
      const v = this.lookup_var("sp_think_mark_collection")
      if (v != null) {
        return JSON5.parse(v)
      }
    },

    // sp_origin_mark_collection の Array 化
    sp_origin_mark_collection_native() {
      const v = this.lookup_var("sp_origin_mark_collection")
      if (v != null) {
        return JSON5.parse(v)
      }
    },
